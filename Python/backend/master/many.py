...
def update(machineIndex,header):  
	...
	r = thisMachine["session"].post((thisMachine["ip"]+"/many"+ADDITION), data=dataMany, headers=header) #thisMachine["session"]
	...
	summary = "\t".join([thisMachine["name"],str(returnJson["counter"]),  str(returnJson["isbot"]), str(returnJson["carrierEngagementCategory"]), str(returnJson["askSentAt"]), str(returnJson["askDur"]), (str(averageDuration)+"-"+thisMachine["name"]), str(returnJson["askStatus"]), str(errorPercentage), str(returnJson["booked"]),str(returnJson["details"])])
...
if __name__ == "__main__": 
	...
	# if len(sys.argv)>2:
	# 	url = sys.argv[1]
	# 	STRINGTOPRINT = sys.argv[2] #.replace("'", '"')
	# 	bigJSON=sys.argv[3] #json.dumps()
	# 	bigJSON=bigJSON.replace("'", '"')
	# 	bigJSON=json.loads(bigJSON)
	# 	# print(STRINGTOPRINT)	
	# 	# print(url)
	# 	# print(bigJSON[0]["puName"])
	# 	# sys.exit()
	# else:
	# 	print("right way is -> nameOfprogram.py url STRINGTOPRINT bigJSON")
	# 	sys.exit()
